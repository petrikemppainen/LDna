...
LDnClusteringOld <- function(snp, mapsnp, nSNPs=1000, w1=10, w2=100 ,LD_threshold1 = 0.1, LD_threshold2 = 0.3, PC_threshold=0.8, verbose=FALSE, plot.tree=FALSE, mc.cores=1, plot.network=NULL, threshold_net=0.9){
  ...
  for(i in unique(mapsnp[,1])){
    ...
    out[[i]] <- mclapply(1:length(Windows), function(b){
      ...
      PCs <- lapply(clusters, function(cl){
        ifelse(length(cl)>1, return(PC_score_old(snp_bin[,snp.id.bin%in%cl], PC_threshold)), return(snp_bin[,snp.id.bin==cl]))
      })
...
PC_score_old <- function(x_A, PC_threshold){
