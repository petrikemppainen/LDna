...
LDnaRaw <- function(LDmat){
    ...
    nE <- rep(0, Ntips)
    tot.d <- NULL
    clusterfile <- matrix(FALSE, Ntips,  tree$Nnode)
    for(i in Ntips:length(tree$edge.length)+1){
        tree.temp <- extract.clade(tree, i)
        nV <- c(nV, length(tree.temp$tip.label))
        LDmat.temp <- LDmat1[colnames(LDmat1) %in% tree.temp$tip.label, rownames(LDmat1) %in% tree.temp$tip.label]
        ...
        temp <- i
        out <- i
        while(any(tree$edge[,1] %in% temp)){
            temp <- tree$edge[,2][tree$edge[,1] %in% temp]
            out <- c(out, temp)
        }
    ...
    rownames(stats) <- c(tree$tip.label, c.names)
    
    d <- tree$edge
    d[,2] <- rownames(stats)[tree$edge[,1]]
    d[,1] <- rownames(stats)[tree$edge[,2]]
