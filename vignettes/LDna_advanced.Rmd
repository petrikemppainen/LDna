...
par(mfcol=c(1,2))
clusters <- extractClusters(ldna, min.edges = 18, phi = 4, rm.COCs = F, plot.tree = TRUE, plot.graph = FALSE)
```
...
par(mfcol=c(1,2))
clusters_4 <- extractClusters(ldna, min.edges = 18, phi = 4, rm.COCs = FALSE, plot.tree = TRUE, plot.graph = TRUE)
clusters_2 <- extractClusters(ldna, min.edges = 18, phi = 2, rm.COCs = FALSE, plot.tree = TRUE, plot.graph = TRUE)
```
...
```{r, fig.width=8, fig.height=4, fig.show='hold', tidy=TRUE}
par(mfcol=c(1,2))
plotLDnetwork(ldna, LDmat, option=2, clusters=clusters_2[c(2,5)], summary=summary, full.network=FALSE, include.parent=TRUE, after.merger=TRUE)
```
