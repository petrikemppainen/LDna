...
Introduction
------------
This vignette provides a tutorial to the **LDna** package with emphasis on demonstrating the basics of the method.
...
```{r fig.width=3, fig.height=3}
g <- plotLDnetwork(LDmat=LDmat, option=1, threshold=0.40)
```
At the core `extractClusters` lies two functions from the package **igraph**: `graph.adjacency` and `plot.igraph`. The below example is used to show how these functions can be used to for instance show all vertex names (i.e locus names) and all LD values for the edges.
```{r fig.width=5, fig.height=5}
...
```
By setting the argument *after.merger=TRUE*, the network is printed at an LD threshold just after merger. Note that the networks are plotted at different LD thresholds compared to above. Overall `extractClusters` is a highly customizable functions which will be further demonstrated in the advanced tutorial. 
```{r fig.width=3, fig.height=3}
